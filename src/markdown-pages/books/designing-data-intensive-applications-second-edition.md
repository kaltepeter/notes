...
A lot of data is unbounded, arriving gradually over time. Often a day late with batching. Batching has finite sizes. Streaming handles more live/up to date data.
...
Equivelent to a record in a batch process.## Stream Processing

- small
- self contained
- immutable object
- contains details
- can be text, json, binary
- generated once by a producer ( publisher, sender)
- potentially consumed by many consumers (subscriber, recipient)
- often grouped by topic or stream
- A file or database could be sufficient, however not designed for it. DBs can be overwhelmed with triggers and more.

### Messaging Systems

- Producers send messages containing events
- Pushed to consumers
- Consumers decide what to do
- Direct communication like a Unix pipe or TCP connection between producer and consumer would be a simple system.
  - most extend to handle topics, failover, etc
...
- What happens if the producers send messages faster than the consumers can process them?
  - drop
  - buffer with a queue
    - important to understand growing queues
  - apply backpressure or flow control
- What happens if a node crashes or temporarily goes offline -- are messages lost?
  - durability may re1quire a combination of writing to disk and or replication
  - if you can lose messages, you can get higher throughput

Direct messaging from producers to consumers

- UDP multicast is used for feeds such as stock market, unreliable but low latency
- Brokerless messaging libraries such as ZeroMQ and nanomsg take a similar approach with TCP or IP multicast
- Some metrics collection agents such as StatsD use unreliable UDP, metrics are only accurate if all messages are received
- If a consumer exposes a service to the network, producers can push via HTTP or RPC

Message Brokers

- wide used
- centralized broker
- some use memory, some persist
- consumers are async
- compare to DB
  - Some use two-phase protocols, such as XA or JTA
  - DB usually keep data until explicit delete. Some brokers auto delete after delivery
  - Most brokers assume working set is small, if it has to buffer a lot of messages it can slow down
  - DB offer secondary indexes and ways to query. Brokers support subscribe
  - querying the DB is a point ini time snapshot. Brokers don't have artitrary query
...
- fanout. 
  - Each message is delievered to all consumers
- can be combined in Kafka

Ackknowlegemnt and Redelivery

- Consumers may crash at anytime
- If a connection to a client is closed or times out without the broker receiving an acknowledgement, it assumes it was not processed. Trys again to another consumer.
  - This can happen if it was processed and ack failed, use atomic commit protocols to protect that
- combined with load balancing
  - consumers process in order received
  - may appear out of order across consumers
  - use a separate queue per consumer, not load balancing to preserve order
- redelivery can waste resources
  - infinite loops
- dead letter queues
  - handle redelivery problems
  - moved to a different queue for consumers
  - setup monitoring
  - operator can permenantly drop them

### Log-based Message Brokers

Sending a packet over a network or making a request to a network service is usually transient and leaves no permenant trace. Log based let's us combine a log with events and allow for reconstructing messages.

A log is append only sequence of records on disk  - infinite loops
- dead letter queues
  - handle redelivery problems
  - moved to a different queue for consumers
  - setup monitoring
...
When you need to parallelize processing and order doesn't matter, AMQP/JMS style makes more sense, though Kafka can support it now.

### Log-based Message Brokers

Sending a packet over a network or making a request to a network service is usually transient and leaves no permenant trace. Log based let's us combine a log with events and allow for reconstructing messages.

- A log is append only sequence of records on disk.
- same structure can implement a broker
- to scale higher throughput, they can be sharded, hosted on different machines
- Kafka calls shards partitions
  - within each partition, the broker assigns a monotonically increasing sequence number or offset, to every mssage
- consumers can pick up the stream anywhere
- trivally supports fan-out messaging, several consumers can read without affecting another
- each client consumes all messages from assigned shards
- number of nodes sharing the work of consuming a topic can be a most the number of log shards in the topic
  - thread pool alternative
- if a single message is slow to process, it holds up the processing of subsequent messages in that shard

Consumer Offsets

- all messages with an offset less than the consumers offset have been processed and all messages with a greater offset have not yet been seen
- broker doesn't need to track ack
- if a consumer fails, it resumes from last recorded offset, which can cause duplicates
- it's similar to a log sequence
- if a consumer fails, another node in the consumer group is assigned and picks up from the last recorded offset
  - if the consumer procesed but didn't record it, it will dupe processing
...
- if a slow consumer tries to catch up after a segment is deleted, messages will be lost
- logs are a bounded buffer size, a circular buffer or ring buffer
- 20TB drive with write throughput of 250 MB/s will take 22 hours to fill disk
- many use object storage to increase capacity

When consumers cannot keep up with producers

- log based approach is a form of buffering with a large but fixed-size buffer (disk space)
- monitor how far a conumer is behind the head of the log and raise an alert if it's significant
- if a consumer falls behind, only that consumer is affected

Replaying old messages

- You can reset the consumer offset to read earlier messages

### Databases and Streams

- we can apply messaging principles to databases
- use an event stream as the system of record for storing data, instead of mutating.
  - model every state change as an immutable log
  - read-optimzied views derived from events
  - log based brokers configured to never delete are well suited for event sourcing

Syncing

- data warehouses use ETL processes
- dual writes if too slow to data dump
  - race conditions
  - unless you have additional concurrency detection, you will not even notice concurrent writes occrured
  - one replicated database with a single leader, the leader determines writes, can preent some issues

Change Data Capture

- change data capture (CDC) is the process of observing all data changes written to a DB and extracting them in a form in which they can be replicated
- call the log consumers derived data systems
- single leader DB
- log based message broker is well sutiedf or transporting the change events from the source database to the destination systems
- logical replicaiton logs can be used
  - schema change challenges
- async

Initial Snapshot

- if you log every change ever made, you can reconstruct state by replaying.
  - too much disk
  - too long
- build a new full text index requires a full copy of the database
  - apply log of recent changes isn't enough
  - use a snapshot

log compaction

- can make records smaller
- 

API support and change streams

- supported by many

CDC vs. Event Sourcing

- In CDC, the application uses the DB in a mutable way>
- In Event Sourcing, the application logic is build on the basis of immutable eventsó
- Though CDC appears easier, it has challenges
  - usually use the upstream schema as a public API which can change
  - decouple internal from external schemas with outbox pattern. Outboxes are tables with theri own schemas, rather than the internal domain
  - developers must maintain the outbox
- event sourcing models user intent

### State, Streams, and Immutability

- if you store changelog durably, the effect is reproducibility
- if you consider the log of events to be your system of record and any mutable state being derived from it, it's easier to reason about the flow through a system

> There is no fundamental need to keep a database at all; the log contains all the information there is. The only reason for storing the database (i.e., the current end-of-the-log) is performance of retrieval operations. -- Jim Gray and Andreas Reuter

### Advantags of Immutable Events

Example: accountants with append only immutable ledger

- if a mistake is made, new records adjust, leaving and audit trail
- capture more than current state

Deriving Several Views from Same Event Log

- optimize for reads
- easier to evolve
- traditional approach to DB is based on a fallacy that the way we query data is the same way we write data.

Concurency Control

- CQRS downside is all async, consuemrs may not see something they wrote
- update read view syncronously with appendign to event log is an option with distributed transactions
- derivingn data simplifies concurrency

Limitations

- high rate of updates or deletes on small dataset grows large
- lot of deletes if legal requires
- crypto-shredding can delete stored encrypted data. the encrypted data is there, but keys are gone

### Processing Streams

- take events and write to DB
- push to usres
...
- Trading systems
- Manufacturering systems
- Military and intelligence systems

Complex Event Processing

- CEP from 90s for analyzing streams
- relationship between query and data is reversed compared to DB. queries are stored, event comes in the engine compares

Stream Analytics

- less focussed on specific events
- measure the rate of a certain type of data
- calculating the rolling average of a value over time
- comparing current stats fto previous time intervals
- computed over fixed time intervals

Maintainning Materialized Views

- stream processor can be used
- DBs are usually inappropiate
  - poor efficiency
  - data freshness
  - possible to write triggers when the computation is incremental
- incremental view maintenance is a more general solution to these problems

Search

- you can index queries, efficiency is in the flip

Event-drive architectures and RPC

- not thought of as stream processors, though overlap
- actor framewokrs are primarily a mechanism for managing concurreny and distributed execution of communicating modules, whereas stream prodcesing is primarily data managmenet
- comms between actors is ephermeral and one-to-one, whereas event logs are durable and multi-subscriber
- actgors can communicate in arbitrary wsays

### Reasoning about Time

- use local system clock on the processing machine to detmerine windowing
- event time vs processing time
  - processing delays
  - confusing them is bad data

Straggler Events

- ignore, track dropped as a metric and alert
- publish a correction

Whose Clock

- log the time the user interaciton occured
- store three timestamps
  - tiem the event occurred, according to the device
  - time the event was sent to the server, according to the device clock
  - time the event was received by the server, according to the server clock
- calc offsets between device and server
- apply offset to the event timestamp

### Windows

- tumbling windows
  - fixed length
  - each event in 1 window
  - round down to nearest unit
- hopping windows
  - fixed length
  - overlap between
- sliding windows
  - all events within certain interval
- session windows
  - no fixed duration
...
- window join
- table join: enrichment
- table join: materialzied views
-  
